...
export const BADGES = [
  ...
  exact('CALENDAR_EXACT', 'Exact Calendar', '\U0001f4c6', 365, 'CALENDAR'),
  { id: 'EXACT_BOOB', label: 'Exact Boob', emoji: '\U0001f34d', fam: 'BOOB', desc: 'Exactly "8008" or "58008".', test: c => c.n === 8008 || c.n === 58008 },
  { id: 'BOOB_80085', label: '80085', emoji: '\U0001f48e', fam: 'BOOB', desc: 'Contains "80085" (spells BOOBS).', test: c => c.s.includes('80085') },
  { id: 'BOOB_8008', label: '8008', emoji: '\U0001f352', fam: 'BOOB', desc: 'Contains "8008" (spells BOOB upside-down).', test: c => c.s.includes('8008') },
  { id: 'HELLO', label: 'Hello', emoji: '\U0001f44b', fam: null, desc: 'Contains "07734" (spells HELLO upside-down).', test: c => c.s.includes('07734') },
  { id: 'SECRET_AGENT', label: 'Secret Agent', emoji: '\U0001f576️', fam: null, desc: 'Contains "007".', test: c => c.s.includes('007') },

  { id: 'ELEVENTH_POWER', label: '11th Power', emoji: '\U0001f52e', fam: 'POWER', desc: 'A perfect eleventh power (n¹¹).', test: c => POWERS[11].has(c.n) },
  { id: 'THIRTEENTH_POWER', label: '13th Power', emoji: '\U0001f9ff', fam: 'POWER', desc: 'A perfect thirteenth power (n¹³).', test: c => POWERS[13].has(c.n) },
  { id: 'SEVENTEENTH_POWER', label: '17th Power', emoji: '\U0001f320', fam: 'POWER', desc: 'A perfect seventeenth power (n¹⁷).', test: c => POWERS[17].has(c.n) },
  { id: 'NINETEENTH_POWER', label: '19th Power', emoji: '☄️', fam: 'POWER', desc: 'A perfect nineteenth power (n¹⁹).', test: c => POWERS[19].has(c.n) },
  { id: 'PRONIC', label: 'Pronic Number', emoji: '\U0001f4d0', fam: null, desc: 'The product of two consecutive integers (n * n+1).', test: c => PRONIC.has(c.n) },

  { id: 'PI', label: 'Pi', emoji: '\U0001f967', fam: 'PI', desc: 'Exactly π (314, 3141, 31415, or 314159).', test: c => c.n === 314 || c.n === 3141 || c.n === 31415 || c.n === 314159 },
  { id: 'PI_CONTAINS_5', label: 'Pi Slice (5)', emoji: '\U0001f95e', fam: 'PI', desc: 'Contains "31415".', test: c => c.s.includes('31415') },
  { id: 'PI_CONTAINS_4', label: 'Pi Slice (4)', emoji: '\U0001f355', fam: 'PI', desc: 'Contains "3141".', test: c => c.s.includes('3141') },
  { id: 'PI_CONTAINS_3', label: 'Pi Slice (3)', emoji: '\U0001f53a', fam: 'PI', desc: 'Contains "314".', test: c => c.s.includes('314') },
  { id: 'E', label: "Euler's Number", emoji: '\U0001f9ee', fam: 'E', desc: 'The number e (271, 2718, 27182, or 271828).', test: c => c.n === 271 || c.n === 2718 || c.n === 27182 || c.n === 271828 },
  { id: 'E_CONTAINS_5', label: 'E Slice (5)', emoji: '\U0001f9ca', fam: 'E', desc: 'Contains "27182".', test: c => c.s.includes('27182') },
  { id: 'E_CONTAINS_4', label: 'E Slice (4)', emoji: '\U0001f4ca', fam: 'E', desc: 'Contains "2718".', test: c => c.s.includes('2718') },
  { id: 'E_CONTAINS_3', label: 'E Slice (3)', emoji: '\U0001f4c8', fam: 'E', desc: 'Contains "271".', test: c => c.s.includes('271') },

  { id: 'ROYAL_FLUSH', label: 'Royal Flush', emoji: '\U0001f451', fam: 'STRAIGHT', desc: 'Contains 56789 — the highest possible straight.', test: c => c.s.includes('56789') },
  { id: 'STRAIGHT_FLUSH', label: 'Straight Flush', emoji: '\U0001f3b4', fam: 'STRAIGHT', desc: 'Contains 5 consecutive same-parity digits (02468, 13579, or their reverse).', test: c => c.s.includes('02468') || c.s.includes('13579') || c.s.includes('86420') || c.s.includes('97531') },
  ...
  { id: 'ALTERNATOR', label: 'Alternator', emoji: '\U0001f503', fam: null, desc: 'Digits strictly alternate between even and odd.', test: c => { if (c.len < 2) return false; const odd = c.d[0] % 2 === 1; for (let i = 0; i < c.len; i++) if ((c.d[i] % 2 === 1) !== ((i % 2 === 0) === odd)) return false; return true; } },

  { id: 'CASCADE', label: 'Cascade', emoji: '\U0001f30a', fam: 'PROGRESSION', desc: 'Every digit increases by exactly 1 from the previous.', test: c => { if (c.len < 2) return false; for (let i = 1; i < c.len; i++) if (c.d[i] !== c.d[i-1] + 1) return false; return true; } },
  { id: 'WATERFALL', label: 'Waterfall', emoji: '\U0001f3de️', fam: 'PROGRESSION', desc: 'Every digit decreases by exactly 1 from the previous.', test: c => { if (c.len < 2) return false; for (let i = 1; i < c.len; i++) if (c.d[i] !== c.d[i-1] - 1) return false; return true; } },
  { id: 'EVEN_SPACING', label: 'Even Spacing', emoji: '\U0001f4cf', fam: 'PROGRESSION', desc: 'All digits are evenly spaced in an arithmetic sequence.', test: c => { if (c.len < 3) return false; const k = c.d[1] - c.d[0]; for (let i = 2; i < c.len; i++) if (c.d[i] - c.d[i-1] !== k) return false; return true; } },
  { id: 'EVEN_SPACING_ABS', label: 'Even Spacing (Absolute)', emoji: '\U0001f4d0', fam: 'PROGRESSION', desc: 'All digits have the same absolute spacing (e.g., ±2 each time).', test: c => { if (c.len < 3) return false; const k = Math.abs(c.d[1] - c.d[0]); for (let i = 2; i < c.len; i++) if (Math.abs(c.d[i] - c.d[i-1]) !== k) return false; return true; } },
  { id: 'TURTLE', label: 'Turtle', emoji: '\U0001f422', fam: 'PROGRESSION', desc: 'All consecutive digits differ by at most 1.', test: c => { if (c.len < 2) return false; for (let i = 1; i < c.len; i++) if (Math.abs(c.d[i] - c.d[i-1]) > 1) return false; return true; } },
  ...
  { id: 'ZIPPER', label: 'Zipper', emoji: '\U0001f9f7', fam: null, desc: 'Two digits alternating perfectly.', test: c => { if (c.len < 2 || c.distinct !== 2) return false; for (let i = 1; i < c.len; i++) if (c.d[i] === c.d[i-1]) return false; return true; } },
  { id: 'HOMOGENEOUS', label: 'Homogeneous', emoji: '\U0001f9ff', fam: null, desc: 'All digits are the same.', test: c => c.len >= 2 && c.distinct === 1 },
  { id: 'HETEROGENEOUS', label: 'Heterogeneous', emoji: '\U0001f308', fam: null, desc: 'No repeated digits.', test: c => c.distinct === c.len },
  { id: 'TRINITY', label: 'Trinity', emoji: '\U0001f531', fam: null, desc: 'Uses exactly three different digits.', test: c => c.distinct === 3 },
  { id: 'QUARTET', label: 'Quartet', emoji: '\U0001f3bb', fam: null, desc: 'Uses exactly four different digits.', test: c => c.distinct === 4 },
  { id: 'BINARY_SOUL', label: 'Binary Soul', emoji: '\U0001f916', fam: null, desc: 'Only 0s and 1s.', test: c => c.d.every(v => v <= 1) },
  { id: 'LOW_BALL', label: 'Low Ball', emoji: '\U0001f41c', fam: null, desc: 'Contains only digits from 0 to 4.', test: c => c.d.every(v => v <= 4) },
  { id: 'HIGH_ROLLER', label: 'High Roller', emoji: '\U0001f3b0', fam: null, desc: 'Contains only digits from 5 to 9.', test: c => c.d.every(v => v >= 5) },
  ...
  { id: 'STROBOGRAMMATIC', label: 'Strobogrammatic', emoji: '\U0001f643', fam: null, desc: 'Looks the same when rotated 180 degrees.', test: c => { let out = ''; for (let i = c.len - 1; i >= 0; i--) { const f = FLIP[c.d[i]]; if (f === undefined) return false; out += f; } return out === c.s; } },

  { id: 'FRAMED_PAIR', label: 'Framed Pair', emoji: '\U0001f5bc️', fam: 'PAIRS', desc: 'A 4-digit number where the middle two digits match each other but differ from both end digits.', test: c => c.len === 4 && c.d[1] === c.d[2] && c.d[0] !== c.d[1] && c.d[3] !== c.d[1] },
  { id: 'FRAMED_TRIPLE', label: 'Framed Triple', emoji: '\U0001f3f5️', fam: 'OF_A_KIND', desc: 'A triple in the middle, bookended by different digits.', test: c => c.len === 5 && c.d[1] === c.d[2] && c.d[2] === c.d[3] && c.d[0] !== c.d[1] && c.d[4] !== c.d[1] },
  { id: 'FRAMED_DOUBLE', label: 'Framed Double', emoji: '\U0001f5c2️', fam: 'PAIRS', desc: 'Two pairs in the middle, bookended by different digits.', test: c => c.len === 6 && c.d[1] === c.d[2] && c.d[3] === c.d[4] && c.d[1] !== c.d[3] && c.d[0] !== c.d[1] && c.d[5] !== c.d[4] },
  { id: 'MIRROR_BOOKENDS', label: 'Mirror Bookends', emoji: '\U0001fa9e', fam: 'BOOKENDS', desc: 'First two digits are reversed as the last two.', test: c => c.len >= 4 && c.d[0] === c.d[c.len - 1] && c.d[1] === c.d[c.len - 2] },
  { id: 'DOUBLE_HOP', label: 'Double Hop', emoji: '\U0001f99f', fam: 'HOPSCOTCH', desc: 'A digit appears at every other position (3 times).', test: c => { if (c.len < 5 || c.distinct < 2) return false; for (let i = 0; i + 4 < c.len; i++) { if (c.d[i] === c.d[i+2] && c.d[i+2] === c.d[i+4]) { const on = i + 6 < c.len && c.d[i+6] === c.d[i], back = i >= 2 && c.d[i-2] === c.d[i]; if (!on && !back) return true; } } return false; } },

  { id: 'CLEAN', label: 'Clean', emoji: '\U0001f9fc', fam: null, desc: 'Ends in a zero.', test: c => c.n % 10 === 0 },
  { id: 'SEMI_CLEAN', label: 'Semi-Clean', emoji: '\U0001fae7', fam: null, desc: 'Ends in a 5.', test: c => c.n % 10 === 5 },
  { id: 'CENTURY', label: 'Century', emoji: '\U0001f4af', fam: null, desc: 'Ends in double zeros.', test: c => c.s.endsWith('00') },
  { id: 'SEMI_CENTURY', label: 'Semi-Century', emoji: '\U0001f31f', fam: null, desc: 'Ends in "50".', test: c => c.n % 100 === 50 },
  { id: 'MILLENNIUM', label: 'Millennium', emoji: '✨', fam: null, desc: 'Ends in triple zeros.', test: c => c.s.endsWith('000') },
  { id: 'SEMI_MILLENNIUM', label: 'Semi-Millennium', emoji: '\U0001f320', fam: null, desc: 'Ends in "500".', test: c => c.n % 1000 === 500 },
  { id: 'SEMI_EPOCH', label: 'Semi-Epoch', emoji: '⏳', fam: null, desc: 'Ends in "5000".', test: c => c.n % 10000 === 5000 },
  { id: 'COLOSSAL', label: 'Colossal', emoji: '\U0001f5ff', fam: null, desc: 'A number greater than 999,000.', test: c => c.n > 999000 },

  { id: 'ELEVEN', label: 'Eleven', emoji: '\U0001f3b1', fam: null, desc: 'Divisible by 11.', test: c => c.n % 11 === 0 },
  { id: 'DOZEN', label: 'Dozen', emoji: '\U0001f95a', fam: null, desc: 'Divisible by 12.', test: c => c.n % 12 === 0 },
  { id: 'LUCKY_SEVEN_DIV', label: 'Lucky Seven (Divisible)', emoji: '\U0001f340', fam: null, desc: 'Divisible by 7.', test: c => c.n % 7 === 0 },
  { id: 'BLACKJACK', label: 'Blackjack', emoji: '♠️', fam: null, desc: 'Digits sum exactly to 21.', test: c => c.sum === 21 },
  { id: 'FEATHER', label: 'Feather', emoji: '\U0001fab6', fam: null, desc: 'The sum of its digits is less than 15.', test: c => c.sum < 15 },
  ...
  { id: 'GAP_ONE', label: 'Gap One', emoji: '\U0001f6aa', fam: null, desc: 'The first and last digits differ by exactly 1.', test: c => c.len >= 2 && Math.abs(c.d[0] - c.d[c.len - 1]) === 1 },
  { id: 'GROUNDED', label: 'Grounded', emoji: '\U0001f6ec', fam: null, desc: 'The first digit is smaller than the last.', test: c => c.len >= 2 && c.d[0] < c.d[c.len - 1] },
  { id: 'LIFTOFF', label: 'Liftoff', emoji: '\U0001f680', fam: null, desc: 'The first digit is larger than the last.', test: c => c.len >= 2 && c.d[0] > c.d[c.len - 1] },
  { id: 'NEIGHBORS', label: 'Neighbors', emoji: '\U0001f3d8️', fam: null, desc: 'Contains two digits that are adjacent in value.', test: c => { for (let i = 0; i + 1 < c.len; i++) if (Math.abs(c.d[i] - c.d[i+1]) === 1) return true; return false; } },
  ...
  { id: 'GHOST', label: 'Ghost', emoji: '\U0001f47b', fam: null, desc: 'Contains exactly one "0".', test: c => c.counts[0] === 1 },
