...
(function () {
  ...
  function detail(c) {
    ...
    h += item(c, 's1c', 'How does it make money on each sale?', 'Gross margin. Understand why it is high or low.',
      '<p class="ans">' + esc(c.q.gm || '') + '</p><div class="grid">' +
    ...
    h += item(c, 's2c', 'Free cash flow', 'Operating cash flow minus capex. Profit can be an accounting opinion; cash is not.',
      '<div class="grid">' + cell('Operating cash flow', money(num(M.ocf), c.cur)) +
    ...
    h += item(c, 's2d', 'Return on invested capital', 'Does reinvested money earn more than it costs?',
      '<div class="grid">' + cell('ROIC', txt(M.roic), null, roic >= 15 ? 'good' : roic !== null && roic < 7 ? 'warn' : null) +
    ...
    h += item(c, 's2e', 'Debt and interest cover', 'Net debt / EBITDA, and EBIT / interest expense.',
      '<div class="grid">' + cell('Total debt', money(num(M.debt), c.cur)) +
    ...
    h += item(c, 's4b', 'Versus its own five-year history', '',
      '<p class="ans">' + (vs === null || vs === undefined
        ? 'Not enough clean history to compare. Check the multiple against the years below yourself.'
        : 'It trades at <strong>' + (vs > 0 ? vs + '% above' : Math.abs(vs) + '% below') + '</strong> its own five-year median P/E of ' + c.pe_med + '.') + '</p>' +
  ...
  function rowHTML(c, idx) {
    return '<article class="row" data-mkt="' + (c.list === 'SG' ? 'SGX' : 'US') + '" data-t="' + esc(c.t) + '">' +
      ...
      '<span class="num strong">' + money(c.mcapN, c.cur) + '<small>Mkt cap</small></span>' +
      '<span class="num">' + txt(c.M.pe) + '<small>P/E</small></span>' +
      '<span class="num c-yield">' + txt(c.M.divy) + '<small>Yield</small></span>' +
  ...
  function sortList(list) {
    ...
    var by = {
      ...
      pe: function (a, b) { return (a.peN === null || a.peN <= 0 ? 1e9 : a.peN) - (b.peN === null || b.peN <= 0 ? 1e9 : b.peN); },
      yield: function (a, b) { return (b.divN || 0) - (a.divN || 0); },
